...
const hosps = datapoints.map(d => +d['Hospitalized cases'])

xPositionScale.domain(d3.extent(dates))
yPositionScale.domain(d3.extent(hosps))
...
//   })
  svg
    ...
    .attr('width', 3)
    .attr('height', d => {
      return height - yPositionScale(d['Hospitalized cases'])
    })
    .attr('x', d => {
      return xPositionScale(d.datetime)
    })
    .attr('y', d => {
      return yPositionScale(d['Hospitalized cases'])
    })
    .attr('fill', 'lightgrey')
    .lower()
    // .attr('class', d => {
...
const xAxis = d3
  ...
  svg
    ...
    d3.select('#toggle4').on('click', () => {
        ...
        const hosps2 = datapoints_30.map(d => +d['Hospitalized cases'])

        
        xPositionScale.domain(d3.extent(dates2))
        yPositionScale.domain(d3.extent(hosps2))
