...
let svg = d3
  ...
  function ready(dummydata) {
    ...
    svg.append('text').attr('class', 'title').text('COVID hospitalizations show second surge').attr('alignment-baseline', 'middle').attr('y',45).attr('font-size', '25px').attr('font-weight', 5).attr('x', 0)
    svg.append('text').attr('class', 'sub-title').text("Click on the buttons below to filter results by time.").attr('alignment-baseline', 'middle').attr('y',75).attr('font-size', '20px').attr('font-weight', 5).attr('x', 0)
