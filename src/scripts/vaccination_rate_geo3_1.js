...
.html(function(d) {
  ...
  return `${d.properties.NAME10} : ${parseFloat(d.percent).toFixed(2)}% <br>
...
// .domain([0, 70])
// .range([['white', 'orange']])
d3.scaleSequential(d3.interpolateOranges).domain([0, 100])
...
Promise.all([
    ...
    d3.json(require('/data/ct_zipcodes.geojson')),
    d3.csv(require('/data/vaccination_clinics.csv')),
    d3.csv(require('/data/vaccination_geo.csv')),
    d3.csv(require('/data/vaccination_geo3.csv')),
    d3.csv(require('/data/zipcodes.csv'))

  ]).then(ready)
  .catch(err => console.log('Failed on', err))


  function ready([json2, json, clinics, geo, geo2, zipcodes]) {
    ...
    svg.append('text').attr('id', 'box1-text').text('0').attr('x',width-220).attr('y', height-200).attr('font-size', 10)
      svg.append('text').attr('id', 'box4-text').text('100%').attr('x',width-95).attr('y', height-200).attr('font-size', 10)

      svg.append('rect').attr('id', 'box0').attr('width', 25).attr('height', 5).attr('x',width-175).attr('y', height-200).attr('fill', colorScale(0))
      svg.append('rect').attr('id', 'box1').attr('width', 25).attr('height', 5).attr('x',width-150).attr('y', height-200).attr('fill', colorScale(25))
      svg.append('rect').attr('id', 'box2').attr('width', 25).attr('height', 5).attr('x',width-125).attr('y', height-200).attr('fill', colorScale(50))
      svg.append('rect').attr('id', 'box3').attr('width', 25).attr('height', 5).attr('x',width-100).attr('y', height-200).attr('fill', colorScale(75))
      svg.append('rect').attr('id', 'box4').attr('width', 25).attr('height', 5).attr('x',width-75).attr('y', height-200).attr('fill', colorScale(100))


    // const zipcode = topojson.feature(json, json.objects.tracts_ct)

   console.log(json, 'zipcodes')
    ...
    d3.select('#toggle').on('click', () => {
      ...
      d3.select('#toggle2').on('click', () => {
        ...
        svg.select('#box1').remove()
        svg.select('#box2').remove()
        svg.select('#box3').remove()
        svg.select('#box4').remove()
        svg.select('#box1-text').remove()
        svg.select('#box4-text').remove()
    ...
    //   })
      ...
      function render() {
        ...
        svg.select('#box4-text').text('100%').attr('x',newWidth-75).attr('y', height-200).attr('font-size', 10)
    ...
    svg.select('#box4').attr('y', height-200)
        ...
        svg.select('#box4-text').text('100%').attr('x',newWidth-75).attr('y', height-400).attr('font-size', 10)
