...
// var tick = new Date()
  ...
  Promise.all([
    //   d3.csv('data/waterfall_dummy_data.csv'),
      d3.csv(require('/data/age_slider.csv')),
...
function ready([datapoints, datapoints_30]) {
    ...
    var subgroups = datapoints.columns.slice(1)

  // List of groups = species here = value of the first column called group -> I show them on the X axis
  var groups = d3.map(datapoints, function(d){return(d.group)}).keys()
  ...
  xPositionScale  .domain(groups)
    ...
    .enter().append("g")
      ...
      .enter().append("rect")
        .attr("x", function(d) { return xPositionScale(d.data.group); })
        .attr("y", function(d) { return yPositionScale(d[1]); })
        .attr("height", function(d) { return yPositionScale(d[0]) - yPositionScale(d[1]); })
