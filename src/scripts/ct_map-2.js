...
Promise.all([
    ...
    d3.csv(require('/data/poverty_ct_towns.csv')),
...
function ready([json, json2, race, housing, single_family, single_family_sales, poverty]) {
...
//bar_4         
         ...
         svg.append('text').attr('id', 'median-voucher').text('0.21').attr('y',45).attr('x', yPositionScale2(20)+(0.4*width)+35).attr('font-weight', 5).attr('font-size',13)
...
//bar_3
         ...
         .attr("height", 20)
         .attr("x",width/35)
         .attr("width",yPositionScale(10.3))
         .attr('fill', 'grey')
         .attr('rx',10)
         .attr('ry',10)


         svg.append('text').attr('id', 'median-sub').text('10.3').attr('y',45).attr('x', yPositionScale2(20)).attr('font-weight', 5).attr('font-size',13)
...
// hispanic
      ...
      poverty.forEach(function(r){if (r.Town===d.properties.NAME10){
        ...
        .transition()
        .duration(700)
        .ease(d3.easeElastic).attr('width', yPositionScale(parseFloat(r['Poverty Status'])))
